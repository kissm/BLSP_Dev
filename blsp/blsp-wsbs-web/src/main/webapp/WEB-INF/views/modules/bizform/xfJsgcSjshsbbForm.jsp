		...
		<form id="xfJsgcSjshsbbForm" action="${ctx}/xfJsgcSjshsbb/save" method="post">
    		<div class="card-body card-padding">
    			<table class="editTable" style="width: 960px;" border="0" cellspacing="0" cellpadding="0">
					<tbody>
						<tr>
							<td class="title" colspan="7" style="font-size: 16px;font-weight: bolder;">
								建设工程消防设计审核申报表
							</td>
						</tr>
						<div id="baseOne">
							...
							<tr>
								...
								<td>
									<input class=" base" type="text" 
										...
										value='<fmt:formatDate value="${formObject.planStartDate}" 
										pattern="yyyy-MM-dd" />' name="planStartDate" />
								</td>
								<td class="title">
									计划竣工日期
								</td>
								<td>
									<input class=" base" type="text" 
										onClick="WdatePicker()" placeholder="点击填日期" 
										value='<fmt:formatDate value="${formObject.planEndDate}" 
										pattern="yyyy-MM-dd" />' name="planEndDate" />
								</td>	
							</tr>
							<tr>
								<td class="title" colspan="2">
								...
								<td class="text-center" colspan="5" style="padding-left: 10px;">
							...
							<tr class="construction" name="unitTr" style="height: 45px;">
							...
							<tr id="construction" class="construction" name="unitTr">
								...
								<td class="title" colspan="2">
									<input type="hidden" name="jId" value="${unit.jId}" />
									施工单位
									<button type="button" onclick="addRow('construction')" class="btn btn-primary"><i class="md add"></i></button>
								</td>
								<td>
									<input type="text"  maxlength="200" value="${unit.unitName}" name="unitName"></input>
								</td>
							...
							<c:if test="${unit.unitType == '设计单位'}">
								<tr class="construction" style="height: 45px;">
									<td style="display: none;">
										<input type="hidden" name="unitId" value="${unit.unitId}" />
									</td>
									<td style="display: none;">
			...
			<c:if test="${unit.unitType == '施工单位' && i.index > 2}">
						<tr id="uni${unit.unitId}" class="construction">
							...
							<td class="title" colspan="2">
								<input type="hidden" name="jId" value="${unit.jId}" />
								施工单位
								<button onclick="removeRow('uni${unit.unitId}')" class="btn btn-danger"><i class="md minus"></i></button>
							</td>
							<td>
								<input type="text"  maxlength="200" value="${unit.unitName}" name="unitName"></input>
							</td>
			...
			<tr>
				<td class="content" colspan="10">
					...
					3．消防设计文件，数量：<input maxlength="50" class="base" style="width:50px;height: auto;border-bottom: 1px solid #ccc;" type="text" value="${formObject.fireMaterial}" name="fireMaterial" />份（大写）；<br/>
					<label class="checkbox checkbox-inline m-l-25">
						<input class="base" type="checkbox" value="D" name="meanwhileSubMat" <c:if test="${formObject.meanwhileSubMat.contains('D')}"> checked </c:if> />
						<i class="input-helper"></i>
					</label> 
					4．专家评审申报材料（特殊消防设计文件，或者设计采用的国际标准、境外消防技术标准的中文文本，其他有关消防设计的应用实例、产品说明等技术资料），数量：<input maxlength="50" class="base" style="width:50px;height: auto;border-bottom: 1px solid #ccc;" type="text" value="${formObject.revMaterial}" name="revMaterial" />份（大写）；<br/>
								<label class="checkbox checkbox-inline m-l-25">
						...
						<tr>
							<td class="content" colspan="10">
								其他需要说明的情况：<br/>
								<textarea maxlength="500" class="form-control auto-size base" rows="5" placeholder="请输入..." style="resize:none;width:940px;border: none;text-indent:2em;" name="otherCondition">${formObject.otherCondition}</textarea>
							</td>
						</tr>
					</div>	
				</table>
