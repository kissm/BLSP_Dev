...
<body>
	<div class="card">
		<div class="card-header">
			<h2>
				政府项目受理列表<small>您可通过本功能进行政府项目列表查询,并可以进行补充信息及材料</small>
			</h2>
			<ul class="actions">
	            <li>
	               <button data-toggle="tooltip" data-placement="bottom" title="新办政府项目受理" class="btn btn-success btn-icon m-r-5" onclick="javascrtpt:window.location.href='${ctx}/project/accpet/index/'"><i class="md md-add"></i></button>
	            </li>
	            <li>
		...
		<form id="searchForm" role="form" action="${ctx}/project/list/"
			method="post">
			...
			<table class="table table-striped table-vmiddle bootgrid-table"
				...
				<tbody>
					<c:forEach items="${page.list}" var="project">
						<tr>
							<td><a
								href="${ctx}/project/view?id=${project.prjInstanceVo.id}">
							...
							<td class="text-center text-nowrap">
								<!-- test="${project.prjStageVo.stageStatus eq '3'}">
								<button class="btn btn-icon"
									onclick="javascrtpt:window.location.href='${ctx}/project/form?prjInstanceVo.id=${project.prjInstanceVo.id}'"
									type="button">
									<span class="md ">下一阶段受理</span>
								</button>
								<-->
