...
<body>
	<div class="card">
		<div class="card-header">
			...
			<ul class="actions">
				<li>
	               <button data-toggle="tooltip" data-placement="bottom" title="新办企业项目受理" class="btn btn-success btn-icon m-r-5" onclick="javascript:window.location.href='${ctx}/enterprise/project/form'"><i class="md md-add"></i></button>
	            </li>
	            <li>
		...
		<form id="searchForm" role="form" action="${ctx}/enterprise/project/list/" method="post">
				...
				<tbody>
					<c:forEach items="${page.list}" var="project">
						<tr>
							...
							<td>${project.prjStageVo.stageStatus eq '0' ?"待资料补齐":(project.prjStageVo.stageStatus eq '4'?"办结":"审批中")}
							</td>
							<td class="text-center text-nowrap">
								...
								<c:if test="${project.prjStageVo.stageStatus eq '0' && project.prjStageDefineVo.preStageId eq '0'}">
									<button class="btn btn-icon btn-warning" title="编辑" data-toggle="tooltip" data-placement="bottom"
										onclick="javascript:window.location.href='${ctx}/enterprise/project/form?id=${project.prjInstanceVo.id}'"
										type="button">
								...
								<c:if test="${project.prjStageVo.stageStatus eq '1' || (project.prjStageVo.stageStatus eq '0' && project.prjStageDefineVo.preStageId ne '0')}">
									<button class="btn btn-icon btn-info m-r-5" title="资料补齐" data-toggle="tooltip" data-placement="bottom"
										onclick="javascript:window.location.href='${ctx}/enterprise/project/form?id=${project.prjInstanceVo.id}&editFlag=1'"
										type="button">
								...
								<c:if test="${not empty project.prjStageDefineVo.nextStageId and project.prjStageVo.stageStatus eq '4'}">
									<button class="btn btn-icon btn-info bgm-blue m-r-5" title="下一阶段受理" data-toggle="tooltip" data-placement="bottom"
										onclick="javascript:window.location.href='${ctx}/enterprise/project/form?id=${project.prjInstanceVo.id}&nextStageId=${project.prjStageDefineVo.nextStageId}&editFlag=1'"
										type="button">
