...
<body>
	...
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="#oftenpass" data-toggle="tab">常旅客</a></li>
		<!--
	...
	<div class="tab-content">
		...
		<div id="linkmans" class="tab-pane ">
		</div>
		<div id="oftenpass" class="tab-pane active">
			<table id="contentTable" class="table table-striped table-bordered table-condensed">
				...
				<c:forEach items="${page.list}" var="user">
					<tr>
						...
						<td><ul>
						<c:forEach items="${user.idcards}" var="card">
								<li>证件类型:
									<c:choose>
										<c:when test="${1==card.idtype}">身份证</c:when>
										<c:when test="${2==card.idtype}">护照</c:when>
									</c:choose>
									证件号:${card.idno}</li>
						</c:forEach>
						</ul>
