...
<script>
	function tijiao() {
		//将页面事项集合所有事项ID获取到，传入后台
		var taskIds = '${taskIds}';
		var d = $("#date").val();
		if(typeof(d) == "undefined"){
			window.location.href = "${ctx}/project/print?id=${project.prjInstanceVo.id}&name=受理反馈通知书.doc&taskIds="+ taskIds;
		}else{
			window.location.href = "${ctx}/project/print?id=${project.prjInstanceVo.id}&name=受理反馈通知书.doc&date="+ d +"&taskIds="+ taskIds;
		}
	}
</script>
</head>
	...
	<div class="card">
		<div class="card-header">
			...
			<ul class="actions">
				...
				<li>
					<button data-toggle="tooltip" data-placement="bottom" title="返回项目列表" class="btn btn-warning btn-icon m-r-5" onclick="javascrtpt:window.location.href='${ctx}/project/list'">
		...
		<div class="btn-demo text-center p-15">
			...
			<button class="btn btn-warning" onclick="javascrtpt:window.location.href='${ctx}/project/list'">返回项目列表</button>
		</div>
	</div>
</body>
