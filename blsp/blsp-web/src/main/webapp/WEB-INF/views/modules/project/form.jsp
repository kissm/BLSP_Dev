		...
		<div class="card-body card-padding">
			<div class="row">
						...
						<div role="tabpanel"
							...
							<form method="post" name="form1" id="form1">
								...
								<input type="hidden" name="prjInstanceVo.priceType"
									...
									value="${project.prjInstanceVo.isGovType}"><input
							...
							<div class="table-responsive">
								<form id="form2" method="post" name="form2">
									<table class="table table-striped table-vmiddle bootgrid-table">
										...
										<tbody>
											<c:if test="${project.prjInstanceVo.id eq null}">
												<c:set var="i" value="-1"></c:set>
												<c:forEach items="${project.prjStageMaterialDefineVoMap}"
													var="map" varStatus="v">
													<c:forEach items="${map.value}" var="mater" varStatus="var">
														...
														<tr>
															...
															<td
																class='text-left text-nowrap ${v.index%2==0?"odd":"even"}'><sys:file
														...
														<tr>
															<c:if test="${var.index eq 0}">
																...
																<td rowspan="${fn:length(map.value)}"
																	...
																	class="checkbox checkbox-inline"><input
																		type="checkbox" value="${map.key}" data-type="taskId" onclick="return false" 
																		data="${map.key}" task-code="${taskDefine.taskCode}" task-status="${taskVo.taskStatus}" 
																		name="prjTaskDefineVoList[${v.index}].id"
																		${taskVo.taskStatus eq '0'?"":"checked"}>
																		<i
																		class="input-helper"></i>${taskDefine.taskName}</label></td>
															</c:if>
															<td class='${v.index%2==0?"odd":"even"}' style="padding-left: 30px;"><c:if
				...
				<div class="btn-demo text-center">
					<c:if
						test="${project.prjStageVo eq null ||project.prjStageVo.stageStatus eq '0'}">
						<button data-toggle="modal" id="acceptButton"
							class="btn btn-primary waves-effect" type="button">受理</button>
						<button id="sa-success" class="btn btn-warning waves-effect"
