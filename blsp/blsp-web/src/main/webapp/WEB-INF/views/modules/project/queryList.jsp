...
<script type="text/javascript">
	...
	function ajaxGetPrjEnd(prjInsId) {
		$('#modalContent').attr('src','${ctx}/project/stop/form?prjInsId='+prjInsId);
		$('.modal-title').text("项目终止页");
		//$('#iframeDIV').css("height","180px");
		//$('#iframeDIV').css("width","400px");
		$('#taskForm').modal('show');
	};
			...
			<table class="table table-striped table-vmiddle bootgrid-table"
				...
				<thead>
					<tr>
						...
						<th class="text-center col-xs-3">项目名称</th>
						<th class="text-center col-xs-2">建设单位名称</th>
				...
				<tbody>
					<c:forEach items="${page.list}" var="project">
						<tr>
							...
							<td class="text-center">${project.prjInstanceVo.company}</td>
							...
							<td class="text-center">
							<c:if test="${project.prjStageVo.stageStatus ne 4 && project.prjInstanceVo.isPrjComplete ne '9'}">
								<shiro:hasPermission name="task:offline:finish">
									...
									<button class="btn btn-info waves-effect" data-toggle="modal" onclick="ajaxGetTask(${project.prjInstanceVo.id},${project.prjStageVo.id});" >未办事项</button>
								</shiro:hasPermission>
								
								<shiro:hasPermission name="project:force:stop">
									<button class="btn btn-info waves-effect" data-toggle="modal" onclick="ajaxGetPrjEnd(${project.prjInstanceVo.id});" >终止</button>
								</shiro:hasPermission>
								</c:if>
							</td>
	...
	<div class="modal fade" data-modal-color="cyan" id="taskForm" data-backdrop="static" data-keyboard="false" tabindex="-1" role="dialog" aria-hidden="true">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div id="iframeDIV" style="height:400px;">
