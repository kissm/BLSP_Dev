 ...
 *
 */
package com.framework.osp.modules.sys.utils;
...
import com.framework.osp.common.service.BaseService;
...
import com.framework.osp.modules.sys.dao.MenuDao;
import com.framework.osp.modules.sys.dao.OfficeDao;
import com.framework.osp.modules.sys.dao.RoleDao;
...
public class UserUtils {
	...
	public static void clearCache(User user) {
		...
		if (user.getOffice() != null && user.getOffice().getId() != null && user.getOffice().getName() != null) {
	 ...
	 * @return
	 */
	public static Office getOffice(String depid) {
		Office office = (Office) getCache(depid + OFFICE_CACHE);
		if (office == null) {
			office = officeDao.get(depid);
		}
		putCache(depid + OFFICE_CACHE, office);
		return office;
	}
	...
	/**
	 * 通过部门ID得到此部门所有用户列表,只有ID与名字
	 *
	 * @param officeId
	 * @return
	 */
	public static List<User> getUsersByOfficeId(String officeId) {
		List<User> userList = userDao.findUserListByOfficeId(officeId);
		return userList;
	}

	/**
	 * 通过父类ID获取此机构下所有部门ID,名称
	 *
	 * @param parentId
	 * @return
	 */
	public static List<Office> findOfficeByParent(String parentId) {
		List<Office> officeList = officeDao.findOfficeByParent(parentId);
		return officeList;
	}
}
