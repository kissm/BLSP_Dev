...
$(document).ready(function(){
	...
	*/
	if($('#chart_id')[0]) {
		...
		if ($('.l-b .l-b-i .l-b-i-l')[0]) {
			$(document).on('click', function (e) {
				if (($(e.target).closest('.l-b .l-b-i .l-b-i-l').length === 0)) {
					$('.l-body:visible').hide(600);
				}else{
					$('.l-body:visible').hide(600);
					if($(e.target).parent().next('.l-body').is(':visible')){
						$(e.target).parent().next('.l-body').hide(600);
					}else{
						$(e.target).parent().next('.l-body').show(600);
					}
				}
			});
		}
		
		if ($('.l-node .l-b-i .l-b-i-l')[0]) {
			$(document).on('click','.l-node .l-b-i .l-b-i-l',function () {
				if($(this).parent('.l-b-i').nextAll('.l-item').is(':visible')){
					$('.l-item:visible').hide(300);
					return;
				}
				$('.l-item:visible').hide(300);
				$(this).parent('.l-b-i').nextAll('.l-item').show(300);
			});
		}
...
function uploadFile(id,file,callFunction,downloadFileName){
